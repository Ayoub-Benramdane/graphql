...
function renderTransactionTemplate(projects) {
    ...
    container.innerHTML = `
            <div class="transaction-container">
                ...
                <div class="transaction-card">
                    <span class="transaction-number">0</span>
                </div>
            </div>
        `;
    return;
  ...
  const width = svg.clientWidth - 180;
  ...
  for (let i = 0; i <= numTicks; i++) {
    ...
    const yPos =
      height - padding - (yValue / totalXp) * (height - 2 * padding) + 70;
  ...
  projects.forEach((project, i) => {
    const cx = padding + (i + 1) * (width / projects.length) + 10;
    const cy =
      height -
      padding -
      ((project.amount + lastXp) / totalXp) * (height - 2 * padding) +
      70;
